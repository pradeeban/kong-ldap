  ...
  request({
    ...
    if (client_id && !error) {
      ...
      if (json_response.data.length == 1) {
        ...
        console.log("get_application_name(): application_name=" + application_name);
      }
      else {
        console.log("get_application_name(): missing client_id or error=" + error);
      }
    }
...
function authorize(client_id, response_type, scope, callback) {
  console.log("authorize(): POST url=" + KONG_API + API_PATH + "/oauth2/authorize");
  console.log("authorize(): form data: client_id=" + client_id);
  console.log("authorize(): form data: response_type=" + response_type);
  console.log("authorize(): form data: scope=" + scope);
  console.log("authorize(): form data: provision_key=" + PROVISION_KEY);
  console.log("authorize(): form data: authenticated_userid=" + "userid123");
  ...
  request({
    ...
    if (error) {
      logger.log("authorize(): got error=" + error);
    }
